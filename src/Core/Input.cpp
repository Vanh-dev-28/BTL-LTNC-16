...
namespace SpaceInvaders
{
    void Input::update()
    {
        ...
        if (Renderer::s_scale > 0.0f)
            {
            mouseX_ = (windowMouseX - Renderer::s_offsetX) / Renderer::s_scale;
            mouseY_ = (windowMouseY - Renderer::s_offsetY) / Renderer::s_scale;
            }
        else
        {
    ...
    bool Input::isMousePressed(Uint32 button) const
    {
        ...
        return (mouseButtons_ & mask) != 0U && (previousMouseButtons_ & mask) == 0U;
    }
