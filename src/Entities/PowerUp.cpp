...
{
    PowerUp::PowerUp(PowerUpType type, float x, float y) : type_(type), x_(x), y_(y), width_(48.0f), height_(48.0f), speed_(100.0f), active_(true)
    {
    }
    ...
    void PowerUp::render(Renderer& renderer) const
    {
        ...
        {
            renderer.fillRect(x_, y_, width_, height_, SDL_Color{255, 165, 0, 255});
        }
    }
    ...
    bool PowerUp::isCollectedBy(float playerX, float playerY, float playerWidth, float playerHeight) const
    {
        if (!active_)
            return false;

        return x_ < playerX + playerWidth && x_ + width_ > playerX && y_ < playerY + playerHeight && y_ + height_ > playerY;
    }
}